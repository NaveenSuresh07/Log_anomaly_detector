...
Unlike clustering algorithms that define "normal" regions, Isolation Forest explicitly targets anomalies. It works by randomly partitioning features — because anomalies are rare and structurally distinct, they are isolated in significantly fewer splits than normal points. This results in faster, more accurate detection in high-dimensional log data.

### Why TF-IDF Vectorization?
Standard word counts often over-emphasize common operational messages (e.g., `Received`, `Starting`). TF-IDF (Term Frequency-Inverse Document Frequency) statistically devalues these high-frequency terms while amplifying rare, high-signal tokens like `Timeout`, `Exception`, `Failed`, and `Refused` that indicate genuine system stress.

### Why Sessionization?
System logs are rarely informative in isolation. An anomaly usually manifests as an unusual sequence of events over time. By grouping logs by a common transaction ID, the model analyzes the entire lifecycle of an operation rather than disconnected fragments — capturing failure patterns that single-line inspection would miss.
